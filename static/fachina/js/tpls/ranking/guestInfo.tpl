  ...
  <div class="ui-container bg-white ranking-action-box clearfix">
    ...
    <div class="ranking-btn-box">
      ...
      {{if result.identity === 'M'}}
        {{if result.adviserStatus === 2}}
          {{if result.isForward === 1}}
            <a data-id="{{ result.joinId }}" href="javascript:;" class="btn btn-orange btn-fixed-middle J-touch J-invite-alone">转发拉票</a>
          {{else}}
            <a href="javascript:J_app.alert('您参加的赛事还没有开赛！');" class="btn btn-orange btn-fixed-middle J-touch">转发拉票</a>
          {{/if}}
        {{else}}
        <a href="javascript:;" class="btn btn-gray btn-fixed-middle disabled">转发拉票</a>
        {{/if}}
      {{else}}
        {{if result.adviserStatus === 2}}
          ...
          {{ else if result.remainVote === 0 }}
            <a data-id="{{ result.joinId }}" data-name="{{result.uName}}" data-img="{{result.uImg}}" href="javascript:;" class="btn btn-red btn-fixed-middle J-touch J-vote-guest">再投1票</a>
          {{ else }}
            <a data-id="{{ result.joinId }}" data-name="{{result.uName}}" data-img="{{result.uImg}}" href="javascript:;" class="btn btn-red btn-fixed-middle J-touch J-vote-guest">投票</a>
          {{ /if }}
        {{else}}
          <a href="javascript:;" class="btn btn-gray btn-fixed-middle disabled">转发拉票</a>
        {{/if}}
